...
fn permute(x: Vec3) -> Vec3 {
    Vec3::modulo(((x * 34.0) + Vec3::broadcast(1.0)) * x, 289.0)
}
...
fn simplex_noise(pos: Vec2) -> f32 {
    let mut i: Vec2 = Vec2::floor(pos + Vec2::broadcast(Vec2::dot(pos, CY)));
    let x0: Vec2 = pos - i + Vec2::broadcast(Vec2::dot(i, CX));
    let i1 = if x0.x > x0.y {
        vec2(1.0, 0.0)
    } else {
        vec2(0.0, 1.0)
    ...
    i.y %= 289.0;
    let p: Vec3 = permute(
        permute(Vec3::broadcast(i.y) + vec3(0.0, i1.y, 1.0))
            + Vec3::broadcast(i.x)
            + vec3(0.0, i1.x, 1.0),
    );
    let mut m: Vec3 = (Vec3::broadcast(0.5)
        - vec3(x0.magnitude2(), x12_lo.magnitude2(), x12_hi.magnitude2()))
    .max(Vec3::ZERO);
    m = m * m;
    m = m * m;
    let x = 2.0 * (p * CW).fract() - Vec3::broadcast(1.0);
    let h = x.abs() - Vec3::broadcast(0.5);
    let ox = Vec3::floor(x + Vec3::broadcast(0.5));
    let a0 = x - ox;
    m *= Vec3::broadcast(1.792_842_9) - Vec3::broadcast(0.853_734_73) * (a0 * a0 + h * h);
    let g: Vec3 = Vec3 {
...
fn fnoise(ampl: f32, in_wv: Vec2) -> f32 {
    let mut dec: Vec2 = Vec2::broadcast(1.0) + in_wv * ampl;
